...
def test_open_S3Upload(config, driver):
    _log.info('Opening S3Upload.ipynb')
    pytest.notebook.open_url(pytest.notebook._url + '/hub/user-redirect/lab/tree/HPECP5.4/DeltaLake/S3Upload.ipynb')



def test_execute_S3Upload(config, driver):
    ...
    flag = True if "File Copied." in outputs else False
    assert flag, "not able to execute S3Upload.ipynb."


def test_open_Submit(config, driver):
    _log.info('Opening Submit.ipynb')
    pytest.notebook.open_url(pytest.notebook._url + '/hub/user-redirect/lab/tree/HPECP5.4/DeltaLake/Submit.ipynb')


def test_execute_Submit(config, driver):
    pytest.notebook.click_all_play()
    pytest.notebook.confirm_dialog()
    endTime = datetime.datetime.now() + datetime.timedelta(minutes=30)
    status = False
    ...
    assert flag, "not able to execute Submit.ipynb."