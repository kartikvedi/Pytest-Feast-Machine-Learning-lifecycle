...
def test_compute_hosts_configure(config, driver, host_passwd):
    ...
    hosts = cluster['master_nodes'] + cluster['worker_nodes']
    hosts = ",".join(hosts)
    driver.get(config['url'] + '/bdswebui/k8s/hosts')
    time.sleep(5)

    _log.info(f"Configure IP range: {hosts}")
    pytest.hosts_page.submit_form(ip_list=hosts, password=host_passwd)
    time.sleep(6)
    ...
    list_hosts_phase1 = pytest.hosts_page.select_hosts(hosts, with_status='phase 1 of 2 completed')
...
def test_compute_hosts_install(config, driver):
    hosts_conf = config['hosts']
    driver.get(config['url'] + '/bdswebui/k8s/hosts')
    time.sleep(5)
